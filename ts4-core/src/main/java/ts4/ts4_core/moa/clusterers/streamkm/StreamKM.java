package ts4.ts4_core.moa.clusterers.streamkm;
...
public class StreamKM extends AbstractClusterer {
	...
	private static final long serialVersionUID = 6380792715390617378L;
    ...
    public double lloydPlusPlus(int k, int n, int d, Point points[], Point centres[]){
    	...
    	tmpCentresStreamingCoreset = chooseRandomCentres(k, n, d, points);
		double cost = targetFunctionValue(k, n, tmpCentresStreamingCoreset, points);
		double newCost = cost;
		...
		do{
			...
			for(i = 0; i < n; i++){
				int centre = points[i].determineClusterCentreKMeans(k,tmpCentresStreamingCoreset);
			...
			//move centres
			for(i=0; i<k; i++){
				for(int l=0; l<tmpCentresStreamingCoreset[i].dimension; l++){
					tmpCentresStreamingCoreset[i].coordinates[l] = massCentres[i].coordinates[l];
					tmpCentresStreamingCoreset[i].weight = numberOfPoints[i];
				}
			}
			
			//calculate costs
			newCost = targetFunctionValue(k, n, tmpCentresStreamingCoreset, points);
